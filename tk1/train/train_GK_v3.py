# %% [markdown]
# # Rice Disease Detection - Enhanced Pipeline with SOTA Techniques
# Improvements: ConvNeXt, EfficientNetV2, MixUp, Label Smoothing, Focal Loss, Attention

# %%
# ===== IMPORTS =====
import os, shutil, random, cv2, torch, gc, time, math, subprocess, sys
import pandas as pd
import numpy as np
import seaborn as sns
import matplotlib.pyplot as plt
import matplotlib.patches as patches
import logging
from datetime import datetime
from pathlib import Path
from PIL import Image
from tqdm import tqdm
from contextlib import nullcontext
from typing import Dict, List, Tuple, Optional
import warnings
warnings.filterwarnings('ignore')

# PyTorch
import torch.nn as nn
import torch.optim as optim
import torch.nn.functional as F
from torch.utils.data import Dataset, DataLoader
from torchvision import transforms, models
from torchvision.models import (
    efficientnet_b0, EfficientNet_B0_Weights,
    efficientnet_v2_s, EfficientNet_V2_S_Weights,
    mobilenet_v2, MobileNet_V2_Weights,
    convnext_tiny, ConvNeXt_Tiny_Weights
)

# Sklearn
from sklearn.model_selection import train_test_split
from sklearn.metrics import accuracy_score, precision_recall_fscore_support, confusion_matrix, classification_report

# AMP
try:
    from torch.amp import GradScaler, autocast
    _NEW_AMP = True
except Exception:
    from torch.cuda.amp import GradScaler, autocast
    _NEW_AMP = False

# %%
# ===== SETUP =====
def get_output_folder(parent_dir: str, env_name: str) -> str:
    os.makedirs(parent_dir, exist_ok=True)
    experiment_id = f"{env_name}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    output_dir = os.path.join(parent_dir, experiment_id)
    os.makedirs(output_dir, exist_ok=True)
    return output_dir

PATH_OUTPUT = get_output_folder("../output", "enhanced_pipeline_v2")

def create_output_structure(base_path):
    folders = [
        "field_images", "yolo_weights", "yolo_crops", "labeled_crops",
        "classification_weights", "results", "plots", "training_curves",
        "evaluation", "comparison", "test_results", "demo_outputs",
        "logs", "exports", "analysis"
    ]
    
    for folder in folders:
        os.makedirs(os.path.join(base_path, folder), exist_ok=True)
    
    return {folder: os.path.join(base_path, folder) for folder in folders}

OUTPUT_DIRS = create_output_structure(PATH_OUTPUT)

def setup_logging(output_path):
    log_file = os.path.join(output_path, "logs", "pipeline.log")
    
    formatter = logging.Formatter(
        '%(asctime)s - %(levelname)s - %(message)s',
        datefmt='%Y-%m-%d %H:%M:%S'
    )
    
    logger = logging.getLogger()
    logger.setLevel(logging.INFO)
    logger.handlers.clear()
    
    console_handler = logging.StreamHandler()
    console_handler.setFormatter(formatter)
    logger.addHandler(console_handler)
    
    file_handler = logging.FileHandler(log_file, mode='w', encoding='utf-8')
    file_handler.setFormatter(formatter)
    logger.addHandler(file_handler)
    
    logging.info(f"Logging initialized: {log_file}")
    return logger

logger = setup_logging(PATH_OUTPUT)

# %%
# ===== DATASET CONFIGURATION =====
LABELS = {
    0: {
        "name": "brown_spot",
        "match_substrings": [
            "../data/rice-disease-dataset/Rice_Leaf_AUG/Brown Spot",
            "../data/rice-leaf-disease-image/Brownspot",
            "../data/rice-leaf-diseases/rice_leaf_diseases/Brown spot",
            "../data/rice-leafs-disease-dataset/RiceLeafsDisease/train/brown_spot",
            "../data/rice-leaf-images/rice_images/_BrownSpot",
            "../data/rice-diseases-image-dataset/RiceDiseaseDataset/train/BrownSpot",
        ]
    },
    1: {
        "name": "leaf_blast",
        "match_substrings": [
            "../data/rice-disease-dataset/Rice_Leaf_AUG/Leaf Blast",
            "../data/rice-leafs-disease-dataset/RiceLeafsDisease/train/leaf_blast",
            "../data/rice-leaf-images/rice_images/_LeafBlast",
            "../data/rice-diseases-image-dataset/RiceDiseaseDataset/train/LeafBlast",
        ]
    },
    2: {
        "name": "leaf_blight",
        "match_substrings": [
            "../data/rice-disease-dataset/Rice_Leaf_AUG/Sheath Blight",
            "../data/rice-leaf-diseases/rice_leaf_diseases/Bacterial leaf blight",
            "../data/rice-leaf-disease-image/Bacterialblight",
            "../data/rice-leafs-disease-dataset/RiceLeafsDisease/train/bacterial_leaf_blight",
        ]
    },
    3: {
        "name": "healthy",
        "match_substrings": [
            "../data/rice-disease-dataset/Rice_Leaf_AUG/Healthy Rice Leaf",
            "../data/rice-leafs-disease-dataset/RiceLeafsDisease/train/healthy",
            "../data/rice-leaf-images/rice_images/_Healthy",
            "../data/rice-diseases-image-dataset/RiceDiseaseDataset/train/Healthy",
        ]
    }
}

DATASET_SOURCES = {
    "rice-disease-dataset": "dataset_1",
    "rice-leaf-disease-image": "dataset_2",
    "rice-leaf-diseases": "dataset_3", 
    "rice-leafs-disease-dataset": "dataset_4",
    "rice-leaf-images": "dataset_5",
    "rice-diseases-image-dataset": "dataset_6"
}

def get_device_info():
    cuda_available = torch.cuda.is_available()
    cuda_count = torch.cuda.device_count() if cuda_available else 0
    
    if cuda_available and cuda_count > 0:
        device = torch.device("cuda")
        device_name = torch.cuda.get_device_name(0)
        logging.info(f"CUDA available: {cuda_available}")
        logging.info(f"CUDA devices: {cuda_count}")
        logging.info(f"Device name: {device_name}")
    else:
        device = torch.device("cpu")
        if cuda_available:
            logging.warning("CUDA available but no devices detected")
        else:
            logging.info("CUDA not available, using CPU")
    
    return device, cuda_available, cuda_count

DEVICE, CUDA_AVAILABLE, CUDA_COUNT = get_device_info()

# %% [markdown]
# ## PHASE 1: YOLO Setup

# %%
def install_ultralytics():
    try:
        import ultralytics
        logging.info("Ultralytics already installed")
        return True
    except ImportError:
        logging.info("Installing ultralytics...")
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "ultralytics"])
            logging.info("Ultralytics installed successfully")
            return True
        except subprocess.CalledProcessError as e:
            logging.error(f"Installation failed: {e}")
            return False

YOLO_AVAILABLE = install_ultralytics()

if YOLO_AVAILABLE:
    from ultralytics import YOLO
    import ultralytics
    logging.info(f"YOLO version: {ultralytics.__version__}")

# %% [markdown]
# ## PHASE 2: Data Collection

# %%
def collect_images_from_path(path: str) -> List[str]:
    if not os.path.exists(path):
        return []
    
    image_extensions = ('.jpg', '.jpeg', '.png', '.JPG', '.JPEG', '.PNG')
    images = []
    
    try:
        for file in os.listdir(path):
            if file.endswith(image_extensions):
                images.append(os.path.join(path, file))
    except Exception as e:
        logging.warning(f"Error reading {path}: {e}")
    
    return images

def auto_collect_dataset_from_labels():
    logging.info("=" * 60)
    logging.info("AUTO-COLLECTING DATASET")
    logging.info("=" * 60)
    
    all_data = []
    
    for label_id, label_info in LABELS.items():
        label_name = label_info['name']
        match_paths = label_info['match_substrings']
        
        logging.info(f"\nCollecting {label_name} (ID: {label_id})...")
        
        for path in match_paths:
            images = collect_images_from_path(path)
            
            if len(images) > 0:
                dataset_source = "unknown"
                for source_key, source_id in DATASET_SOURCES.items():
                    if source_key in path:
                        dataset_source = source_id
                        break
                
                logging.info(f"  ✓ {len(images)} images from {path}")
                
                for img_path in images:
                    all_data.append({
                        'image_path': img_path,
                        'label_id': label_id,
                        'label_name': label_name,
                        'dataset_source': dataset_source,
                        'source_path': path
                    })
            else:
                logging.info(f"  ✗ No images in {path}")
    
    df = pd.DataFrame(all_data)
    
    logging.info(f"\nTotal: {len(df)} images")
    logging.info(f"By label:\n{df.groupby(['label_id', 'label_name']).size()}")
    logging.info(f"By source:\n{df.groupby('dataset_source').size()}")
    
    return df

collected_df = auto_collect_dataset_from_labels()

collection_csv = os.path.join(OUTPUT_DIRS["results"], "collected_images.csv")
collected_df.to_csv(collection_csv, index=False)

# %%
def plot_data_distribution(df, save_dir):
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Dataset Collection Overview', fontsize=16, fontweight='bold')
    
    ax = axes[0, 0]
    label_counts = df.groupby('label_name').size().sort_values(ascending=True)
    label_counts.plot(kind='barh', ax=ax, color='steelblue')
    ax.set_xlabel('Number of Images')
    ax.set_title('Images per Disease Category')
    ax.grid(True, alpha=0.3, axis='x')
    
    ax = axes[0, 1]
    source_counts = df.groupby('dataset_source').size().sort_values(ascending=False)
    ax.bar(range(len(source_counts)), source_counts.values, color='coral')
    ax.set_xticks(range(len(source_counts)))
    ax.set_xticklabels(source_counts.index, rotation=45, ha='right')
    ax.set_ylabel('Number of Images')
    ax.set_title('Images per Dataset Source')
    ax.grid(True, alpha=0.3, axis='y')
    
    ax = axes[1, 0]
    pivot = df.groupby(['label_name', 'dataset_source']).size().unstack(fill_value=0)
    pivot.plot(kind='bar', stacked=True, ax=ax, colormap='Set3')
    ax.set_xlabel('Disease Category')
    ax.set_ylabel('Number of Images')
    ax.set_title('Label Distribution Across Sources')
    ax.legend(title='Source', bbox_to_anchor=(1.05, 1), loc='upper left')
    ax.grid(True, alpha=0.3, axis='y')
    plt.setp(ax.xaxis.get_majorticklabels(), rotation=45, ha='right')
    
    ax = axes[1, 1]
    label_counts = df.groupby('label_name').size()
    colors = plt.cm.Set3(range(len(label_counts)))
    ax.pie(label_counts.values, labels=label_counts.index, autopct='%1.1f%%',
           colors=colors, startangle=90)
    ax.set_title('Overall Label Distribution')
    
    plt.tight_layout()
    save_path = os.path.join(save_dir, 'data_distribution.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.show()

plot_data_distribution(collected_df, OUTPUT_DIRS["plots"])

# %% [markdown]
# ## PHASE 3: Prepare Dataset (Train/Val/Test)

# %%
def prepare_mock_field_dataset_with_test(df, output_dir, val_size=0.15, test_size=0.15):
    """Create train/val/test splits"""
    logging.info("\n" + "=" * 60)
    logging.info("CREATING TRAIN/VAL/TEST SPLITS")
    logging.info("=" * 60)
    
    trainval_df, test_df = train_test_split(
        df, test_size=test_size, random_state=42, stratify=df['label_id']
    )
    
    train_df, val_df = train_test_split(
        trainval_df, test_size=val_size/(1-test_size), random_state=42,
        stratify=trainval_df['label_id']
    )
    
    logging.info(f"Train: {len(train_df)} ({len(train_df)/len(df)*100:.1f}%)")
    logging.info(f"Val:   {len(val_df)} ({len(val_df)/len(df)*100:.1f}%)")
    logging.info(f"Test:  {len(test_df)} ({len(test_df)/len(df)*100:.1f}%)")
    
    field_data = {'train': [], 'val': [], 'test': []}
    
    for split, split_df in [('train', train_df), ('val', val_df), ('test', test_df)]:
        split_dir = os.path.join(output_dir, split)
        os.makedirs(split_dir, exist_ok=True)
        
        logging.info(f"\nProcessing {split}...")
        
        for idx, row in tqdm(split_df.iterrows(), total=len(split_df), desc=f"Creating {split}"):
            src = row['image_path']
            dst = os.path.join(split_dir, f"{split}_{idx:06d}.jpg")
            
            try:
                shutil.copy2(src, dst)
                
                label_file = os.path.join(split_dir, f"{split}_{idx:06d}.txt")
                with open(label_file, 'w') as f:
                    f.write(f"{row['label_id']} 0.5 0.5 1.0 1.0\n")
                
                field_data[split].append({
                    'image_path': dst,
                    'label_path': label_file,
                    'true_label': row['label_name'],
                    'label_id': row['label_id'],
                    'dataset_source': row['dataset_source']
                })
                
            except Exception as e:
                logging.warning(f"Error: {e}")
                continue
    
    return field_data

def create_yolo_yaml(data_root, output_path):
    import yaml
    
    yaml_data = {
        'path': str(Path(data_root).absolute()),
        'train': 'train',
        'val': 'val',
        'test': 'test',
        'nc': len(LABELS),
        'names': [LABELS[i]['name'] for i in sorted(LABELS.keys())]
    }
    
    yaml_path = os.path.join(output_path, "data.yaml")
    with open(yaml_path, 'w') as f:
        yaml.dump(yaml_data, f, default_flow_style=False)
    
    logging.info(f"Created data.yaml: {yaml_path}")
    return yaml_path

field_data = prepare_mock_field_dataset_with_test(
    collected_df, OUTPUT_DIRS["field_images"], val_size=0.15, test_size=0.15
)

yaml_path = create_yolo_yaml(OUTPUT_DIRS["field_images"], OUTPUT_DIRS["field_images"])

# %% [markdown]
# ## PHASE 4: YOLO Training

# %%
def train_yolo_detector(yaml_path, output_dir, epochs=30, imgsz=640, batch=16):
    if not YOLO_AVAILABLE:
        logging.error("YOLO not available")
        return None
    
    logging.info("\n" + "=" * 60)
    logging.info("YOLO TRAINING")
    logging.info("=" * 60)
    
    yolo_device = '0' if (CUDA_AVAILABLE and CUDA_COUNT > 0) else 'cpu'
    logging.info(f"Training on: {yolo_device}")
    
    model = YOLO('yolov8n.pt')
    
    results = model.train(
        data=yaml_path,
        epochs=epochs,
        imgsz=imgsz,
        batch=batch,
        device=yolo_device,
        patience=10,
        save_period=5,
        workers=4,
        project=output_dir,
        name='rice_detector',
        exist_ok=True,
        verbose=True,
        plots=True
    )
    
    best_model = Path(output_dir) / 'rice_detector' / 'weights' / 'best.pt'
    logging.info(f"✓ Best model: {best_model}")
    
    return str(best_model)

if field_data and len(field_data['train']) > 0:
    best_yolo_model = train_yolo_detector(
        yaml_path=yaml_path,
        output_dir=OUTPUT_DIRS["yolo_weights"],
        epochs=30,
        imgsz=640,
        batch=16
    )
else:
    logging.error("No training data")
    best_yolo_model = None

# %% [markdown]
# ## PHASE 5: Extract Crops

# %%
def extract_crops_with_yolo(yolo_model_path, field_images, output_dir, confidence=0.25):
    logging.info("\n" + "=" * 60)
    logging.info("EXTRACTING CROPS")
    logging.info("=" * 60)
    
    model = YOLO(yolo_model_path)
    crops_data = []
    
    for split in ['train', 'val', 'test']:
        split_dir = os.path.join(output_dir, split)
        os.makedirs(split_dir, exist_ok=True)
        
        logging.info(f"\nProcessing {split}...")
        
        for item in tqdm(field_images[split], desc=f"Extracting {split}"):
            img_path = item['image_path']
            
            try:
                img = cv2.imread(img_path)
                if img is None:
                    logging.warning(f"Cannot read image: {img_path}")
                    continue
                img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                
                results = model.predict(img_path, conf=confidence, verbose=False)
                
                base_name = Path(img_path).stem
                
                for idx, result in enumerate(results[0].boxes):
                    x1, y1, x2, y2 = map(int, result.xyxy[0].cpu().numpy())
                    conf = float(result.conf.cpu())
                    
                    padding = 10
                    h, w = img_rgb.shape[:2]
                    x1 = max(0, x1 - padding)
                    y1 = max(0, y1 - padding)
                    x2 = min(w, x2 + padding)
                    y2 = min(h, y2 + padding)
                    
                    crop = img_rgb[y1:y2, x1:x2]
                    
                    if crop.size == 0 or crop.shape[0] < 30 or crop.shape[1] < 30:
                        continue
                    
                    crop_filename = f"{base_name}_crop{idx:03d}.jpg"
                    crop_path = os.path.join(split_dir, crop_filename)
                    
                    Image.fromarray(crop).save(crop_path)
                    
                    crops_data.append({
                        'crop_path': crop_path,
                        'field_source': img_path,
                        'split': split,
                        'bbox': (x1, y1, x2, y2),
                        'confidence': conf,
                        'crop_id': f"{base_name}_crop{idx:03d}",
                        'true_label': item.get('true_label'),
                        'label_id': item.get('label_id')
                    })
            except Exception as e:
                logging.error(f"Error extracting crops from {img_path}: {e}")
                continue
    
    crops_df = pd.DataFrame(crops_data)
    crops_csv = os.path.join(output_dir, "crops_metadata.csv")
    crops_df.to_csv(crops_csv, index=False)
    
    logging.info(f"\nExtracted {len(crops_df)} crops")
    for split in ['train', 'val', 'test']:
        count = len(crops_df[crops_df['split']==split])
        logging.info(f"  {split}: {count}")
    
    return crops_df

if best_yolo_model and os.path.exists(best_yolo_model):
    crops_df = extract_crops_with_yolo(
        yolo_model_path=best_yolo_model,
        field_images=field_data,
        output_dir=OUTPUT_DIRS["yolo_crops"],
        confidence=0.25
    )
else:
    crops_df = None

# %% [markdown]
# ## PHASE 6: Auto-Label Crops

# %%
def auto_label_crops(crops_df):
    logging.info("\n" + "=" * 60)
    logging.info("AUTO-LABELING")
    logging.info("=" * 60)
    
    labeled_df = crops_df.copy()
    labeled_df['label_name'] = labeled_df['true_label']
    labeled_df = labeled_df.dropna(subset=['label_id'])
    labeled_df['label_id'] = labeled_df['label_id'].astype(int)
    
    logging.info(f"Labeled: {len(labeled_df)} crops")
    logging.info(f"\nDistribution:\n{labeled_df.groupby(['split', 'label_name']).size()}")
    
    labels_csv = os.path.join(OUTPUT_DIRS["labeled_crops"], "labeled_crops.csv")
    labeled_df.to_csv(labels_csv, index=False)
    
    return labeled_df

if crops_df is not None:
    labeled_crops_df = auto_label_crops(crops_df)
else:
    labeled_crops_df = None

# %% [markdown]
# ## PHASE 7: Enhanced Models & Loss Functions

# %%
# ===== SPATIAL ATTENTION MODULE =====
class SpatialAttention(nn.Module):
    """Attention to focus on diseased regions"""
    def __init__(self, in_channels):
        super().__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, in_channels // 8, 1),
            nn.BatchNorm2d(in_channels // 8),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 8, 1, 1),
            nn.Sigmoid()
        )
    
    def forward(self, x):
        attention = self.conv(x)
        return x * attention

# ===== LABEL SMOOTHING LOSS =====
class LabelSmoothingCrossEntropy(nn.Module):
    """Label smoothing for better generalization"""
    def __init__(self, epsilon=0.1):
        super().__init__()
        self.epsilon = epsilon
    
    def forward(self, pred, target):
        n_classes = pred.size(-1)
        log_preds = F.log_softmax(pred, dim=-1)
        loss = -log_preds.sum(dim=-1).mean()
        nll = F.nll_loss(log_preds, target)
        return (1 - self.epsilon) * nll + self.epsilon * (loss / n_classes)

# ===== FOCAL LOSS =====
class FocalLoss(nn.Module):
    """Focal loss for imbalanced data"""
    def __init__(self, alpha=0.25, gamma=2.0):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
    
    def forward(self, inputs, targets):
        ce_loss = F.cross_entropy(inputs, targets, reduction='none')
        pt = torch.exp(-ce_loss)
        focal_loss = self.alpha * (1 - pt) ** self.gamma * ce_loss
        return focal_loss.mean()

# ===== SOTA MODEL BUILDERS =====
def build_convnext(num_classes):
    """ConvNeXt - SOTA 2022"""
    weights = ConvNeXt_Tiny_Weights.DEFAULT
    model = convnext_tiny(weights=weights)
    model.classifier[2] = nn.Linear(model.classifier[2].in_features, num_classes)
    return model

def build_efficientnetv2(num_classes):
    """EfficientNetV2 - Faster than V1"""
    weights = EfficientNet_V2_S_Weights.DEFAULT
    model = efficientnet_v2_s(weights=weights)
    model.classifier[1] = nn.Linear(model.classifier[1].in_features, num_classes)
    return model

def build_efficientnet(num_classes):
    """EfficientNetB0 - Baseline"""
    weights = EfficientNet_B0_Weights.DEFAULT
    model = efficientnet_b0(weights=weights)
    in_feat = model.classifier[1].in_features
    model.classifier[1] = nn.Sequential(
        nn.Dropout(0.2),
        nn.Linear(in_feat, num_classes)
    )
    return model

def build_mobilenet(num_classes):
    """MobileNetV2 - Lightweight"""
    weights = MobileNet_V2_Weights.DEFAULT
    model = mobilenet_v2(weights=weights)
    in_feat = model.classifier[1].in_features
    model.classifier[1] = nn.Sequential(
        nn.Dropout(0.2),
        nn.Linear(in_feat, num_classes)
    )
    return model

# ===== MIXUP DATASET =====
class MixUpDataset(Dataset):
    """MixUp augmentation for better generalization"""
    def __init__(self, base_dataset, alpha=0.2):
        self.dataset = base_dataset
        self.alpha = alpha
    
    def __len__(self):
        return len(self.dataset)
    
    def __getitem__(self, idx):
        img1, label1 = self.dataset[idx]
        
        if random.random() < 0.5:  # 50% chance of mixup
            idx2 = random.randint(0, len(self.dataset) - 1)
            img2, label2 = self.dataset[idx2]
            
            lam = np.random.beta(self.alpha, self.alpha)
            mixed_img = lam * img1 + (1 - lam) * img2
            
            return mixed_img, label1, label2, lam
        else:
            return img1, label1, label1, 1.0

# ===== BASE DATASET =====
class CropDataset(Dataset):
    def __init__(self, df, transform=None):
        self.df = df.reset_index(drop=True)
        self.transform = transform

    def __len__(self):
        return len(self.df)

    def __getitem__(self, idx):
        row = self.df.iloc[idx]
        img = Image.open(row["crop_path"]).convert("RGB")
        label = int(row["label_id"])
        
        if self.transform:
            img = self.transform(img)
        
        return img, label

# ===== TRANSFORMS =====
train_transform = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.RandomResizedCrop(224, scale=(0.8, 1.0)),
    transforms.RandomHorizontalFlip(p=0.5),
    transforms.RandomVerticalFlip(p=0.3),
    transforms.RandomRotation(30),
    transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.2),
    transforms.RandomAffine(degrees=0, translate=(0.1, 0.1)),
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
])

val_transform = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
])

# ===== SETUP AMP =====
def setup_amp():
    use_cuda = CUDA_AVAILABLE and CUDA_COUNT > 0
    device = DEVICE
    
    if not use_cuda:
        amp_ctx = nullcontext()
    else:
        if _NEW_AMP:
            amp_ctx = autocast(device_type="cuda", enabled=True)
        else:
            amp_ctx = autocast(enabled=True)
    
    if _NEW_AMP:
        scaler = GradScaler(device="cuda" if use_cuda else "cpu", enabled=use_cuda)
    else:
        scaler = GradScaler(enabled=use_cuda)
    
    return device, amp_ctx, scaler

# ===== MIXUP LOSS =====
def mixup_criterion(pred, y1, y2, lam):
    return lam * F.cross_entropy(pred, y1) + (1-lam) * F.cross_entropy(pred, y2)

# ===== ENHANCED TRAINING =====
def train_classification_model(model, train_loader, val_loader, epochs=20, lr=1e-4, 
                               model_name="model", use_mixup=True, use_label_smoothing=True):
    device, amp_ctx, scaler = setup_amp()
    model = model.to(device)
    
    # Choose loss function
    if use_label_smoothing:
        criterion = LabelSmoothingCrossEntropy(epsilon=0.1)
        logging.info("Using Label Smoothing")
    else:
        criterion = nn.CrossEntropyLoss()
    
    optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=5, T_mult=2)
    
    best_val_loss = float('inf')
    history = {"train_loss": [], "train_acc": [], "val_loss": [], "val_acc": []}
    best_model_path = None
    
    logging.info(f"\nTraining {model_name}...")
    
    for epoch in range(1, epochs + 1):
        # Train
        model.train()
        train_loss, train_correct, train_total = 0.0, 0, 0
        
        for batch in tqdm(train_loader, desc=f"Epoch {epoch}/{epochs}", leave=False):
            if len(batch) == 4:  # MixUp
                imgs, labels1, labels2, lam = batch
                imgs = imgs.to(device)
                labels1 = labels1.long().to(device)
                labels2 = labels2.long().to(device)
                
                optimizer.zero_grad()
                with amp_ctx:
                    outputs = model(imgs)
                    loss = mixup_criterion(outputs, labels1, labels2, lam)
                
                scaler.scale(loss).backward()
                scaler.step(optimizer)
                scaler.update()
                
                train_loss += loss.item() * imgs.size(0)
                train_correct += (lam * (outputs.argmax(1) == labels1).float() + 
                                (1-lam) * (outputs.argmax(1) == labels2).float()).sum().item()
                train_total += imgs.size(0)
            else:  # Normal
                imgs, labels = batch
                imgs = imgs.to(device)
                labels = labels.long().to(device)
                
                optimizer.zero_grad()
                with amp_ctx:
                    outputs = model(imgs)
                    loss = criterion(outputs, labels)
                
                scaler.scale(loss).backward()
                scaler.step(optimizer)
                scaler.update()
                
                train_loss += loss.item() * imgs.size(0)
                train_correct += (outputs.argmax(1) == labels).sum().item()
                train_total += imgs.size(0)
        
        # Validation
        model.eval()
        val_loss, val_correct, val_total = 0.0, 0, 0
        
        with torch.no_grad():
            for imgs, labels in val_loader:
                imgs = imgs.to(device)
                labels = labels.long().to(device)
                
                with amp_ctx:
                    outputs = model(imgs)
                    loss = F.cross_entropy(outputs, labels)
                
                val_loss += loss.item() * imgs.size(0)
                val_correct += (outputs.argmax(1) == labels).sum().item()
                val_total += imgs.size(0)
        
        train_loss /= train_total
        train_acc = train_correct / train_total
        val_loss /= val_total
        val_acc = val_correct / val_total
        
        history["train_loss"].append(train_loss)
        history["train_acc"].append(train_acc)
        history["val_loss"].append(val_loss)
        history["val_acc"].append(val_acc)
        
        logging.info(f"Epoch {epoch}: TL={train_loss:.4f} TA={train_acc:.4f} VL={val_loss:.4f} VA={val_acc:.4f}")
        
        scheduler.step()
        
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            best_model_path = os.path.join(OUTPUT_DIRS["classification_weights"], f"{model_name}_best.pth")
            torch.save(model.state_dict(), best_model_path)
    
    return history, best_model_path

def count_params(model):
    return sum(p.numel() for p in model.parameters()) / 1e6

@torch.no_grad()
def measure_latency(model, device, input_size=(1, 3, 224, 224), runs=50):
    model.eval()
    x = torch.randn(*input_size, device=device)
    
    for _ in range(10):
        _ = model(x)
    
    if device.type == 'cuda':
        torch.cuda.synchronize()
    
    start = time.time()
    for _ in range(runs):
        _ = model(x)
    
    if device.type == 'cuda':
        torch.cuda.synchronize()
    
    return (time.time() - start) / runs * 1000

@torch.no_grad()
def evaluate_model(model, loader, device):
    model = model.to(device)
    model.eval()
    all_preds, all_labels, all_probs = [], [], []
    
    for imgs, labels in loader:
        imgs = imgs.to(device)
        labels = labels.long().to(device)
        
        outputs = model(imgs)
        probs = F.softmax(outputs, dim=1)
        preds = outputs.argmax(1)
        
        all_preds.append(preds.cpu().numpy())
        all_labels.append(labels.cpu().numpy())
        all_probs.append(probs.cpu().numpy())
    
    y_pred = np.concatenate(all_preds)
    y_true = np.concatenate(all_labels)
    y_probs = np.concatenate(all_probs)
    
    acc = accuracy_score(y_true, y_pred)
    p, r, f1, _ = precision_recall_fscore_support(y_true, y_pred, average='macro', zero_division=0)
    cm = confusion_matrix(y_true, y_pred)
    
    return {
        'accuracy': acc,
        'precision': p,
        'recall': r,
        'f1': f1,
        'confusion_matrix': cm,
        'y_true': y_true,
        'y_pred': y_pred,
        'y_probs': y_probs
    }

# %% [markdown]
# ## PHASE 8: Train Models

# %%
if labeled_crops_df is not None and len(labeled_crops_df) > 0:
    logging.info("\n" + "=" * 60)
    logging.info("ENHANCED CLASSIFICATION TRAINING")
    logging.info("=" * 60)
    
    train_crops = labeled_crops_df[labeled_crops_df['split'] == 'train']
    val_crops = labeled_crops_df[labeled_crops_df['split'] == 'val']
    
    # Base dataset
    train_dataset_base = CropDataset(train_crops, transform=train_transform)
    val_dataset = CropDataset(val_crops, transform=val_transform)
    
    # MixUp dataset
    train_dataset = MixUpDataset(train_dataset_base, alpha=0.2)
    
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False, num_workers=4, pin_memory=True)
    
    num_classes = len(LABELS)
    models_to_train = {
        'ConvNeXt_Tiny': lambda: build_convnext(num_classes),
        'EfficientNetV2_S': lambda: build_efficientnetv2(num_classes),
        'EfficientNetB0': lambda: build_efficientnet(num_classes),
        'MobileNetV2': lambda: build_mobilenet(num_classes)
    }
    
    results = []
    histories = {}
    
    for model_name, model_builder in models_to_train.items():
        logging.info(f"\n{'='*60}")
        logging.info(f"Training {model_name}")
        logging.info(f"{'='*60}")
        
        model = model_builder()
        
        history, best_path = train_classification_model(
            model=model,
            train_loader=train_loader,
            val_loader=val_loader,
            epochs=25,
            lr=1e-4,
            model_name=model_name,
            use_mixup=True,
            use_label_smoothing=True
        )
        
        histories[model_name] = history
        
        model.load_state_dict(torch.load(best_path, map_location=DEVICE))
        model = model.to(DEVICE)
        metrics = evaluate_model(model, val_loader, DEVICE)
        
        params = count_params(model)
        latency = measure_latency(model, DEVICE)
        fps = 1000 / latency
        
        results.append({
            'Model': model_name,
            'Accuracy': metrics['accuracy'],
            'Precision': metrics['precision'],
            'Recall': metrics['recall'],
            'F1': metrics['f1'],
            'Params (M)': params,
            'Latency (ms)': latency,
            'FPS': fps,
            'Checkpoint': best_path
        })
        
        logging.info(f"\n{model_name} Results:")
        logging.info(f"  Accuracy: {metrics['accuracy']:.4f}")
        logging.info(f"  F1: {metrics['f1']:.4f}")
        logging.info(f"  Speed: {fps:.1f} FPS")
        
        del model
        torch.cuda.empty_cache()
        gc.collect()
    
    results_df = pd.DataFrame(results)
    results_csv = os.path.join(OUTPUT_DIRS["results"], "model_comparison.csv")
    results_df.to_csv(results_csv, index=False)
    
    logging.info(f"\n✓ Results saved: {results_csv}")

else:
    results_df = None
    histories = {}

# %% [markdown]
# ## PHASE 9: Per-Class Analysis

# %%
def analyze_per_class_performance(metrics, class_names, save_dir):
    """Detailed per-class analysis"""
    cm = metrics['confusion_matrix']
    
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    
    # Normalized confusion matrix
    ax = axes[0]
    cm_norm = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis]
    sns.heatmap(cm_norm, annot=True, fmt='.2f', cmap='Blues', 
                xticklabels=class_names, yticklabels=class_names, ax=ax)
    ax.set_title('Normalized Confusion Matrix')
    ax.set_xlabel('Predicted')
    ax.set_ylabel('True')
    
    # Per-class metrics
    ax = axes[1]
    p, r, f1, support = precision_recall_fscore_support(
        metrics['y_true'], metrics['y_pred'], labels=range(len(class_names))
    )
    
    x = np.arange(len(class_names))
    width = 0.25
    ax.bar(x - width, p, width, label='Precision', alpha=0.8)
    ax.bar(x, r, width, label='Recall', alpha=0.8)
    ax.bar(x + width, f1, width, label='F1', alpha=0.8)
    
    ax.set_xticks(x)
    ax.set_xticklabels(class_names, rotation=45, ha='right')
    ax.set_ylabel('Score')
    ax.set_title('Per-Class Metrics')
    ax.set_ylim(0, 1.1)
    ax.legend()
    ax.grid(True, alpha=0.3, axis='y')
    
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'per_class_analysis.png'), dpi=150)
    plt.show()
    
    # Log confusion patterns
    logging.info("\nMost confused pairs:")
    for i in range(len(class_names)):
        for j in range(len(class_names)):
            if i != j and cm[i, j] > cm[i].sum() * 0.1:
                logging.info(f"  {class_names[i]} → {class_names[j]}: {cm[i,j]} ({cm[i,j]/cm[i].sum()*100:.1f}%)")

def analyze_confidence_calibration(metrics, save_dir):
    """Check if confidence matches accuracy"""
    y_true = metrics['y_true']
    y_probs = metrics['y_probs']
    
    confidences = np.max(y_probs, axis=1)
    predictions = np.argmax(y_probs, axis=1)
    corrects = (predictions == y_true).astype(int)
    
    # Calibration curve
    bins = np.linspace(0, 1, 11)
    bin_accs = []
    bin_confs = []
    
    for i in range(len(bins)-1):
        mask = (confidences >= bins[i]) & (confidences < bins[i+1])
        if mask.sum() > 0:
            bin_accs.append(corrects[mask].mean())
            bin_confs.append(confidences[mask].mean())
    
    plt.figure(figsize=(8, 6))
    plt.plot([0, 1], [0, 1], 'k--', label='Perfect Calibration')
    plt.plot(bin_confs, bin_accs, 'o-', linewidth=2, markersize=8, label='Model')
    plt.xlabel('Confidence', fontsize=12)
    plt.ylabel('Accuracy', fontsize=12)
    plt.title('Confidence Calibration Curve', fontsize=14, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'confidence_calibration.png'), dpi=150)
    plt.show()
    
    # ECE
    ece = np.mean(np.abs(np.array(bin_confs) - np.array(bin_accs)))
    logging.info(f"Expected Calibration Error (ECE): {ece:.4f}")

# %% [markdown]
# ## PHASE 10: Test Set Evaluation

# %%
def evaluate_on_test_set(results_df, labeled_crops_df, save_dir):
    """Evaluate all models on test set"""
    if results_df is None or labeled_crops_df is None:
        return
    
    logging.info("\n" + "=" * 60)
    logging.info("TEST SET EVALUATION")
    logging.info("=" * 60)
    
    test_crops = labeled_crops_df[labeled_crops_df['split'] == 'test']
    test_dataset = CropDataset(test_crops, transform=val_transform)
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False, num_workers=4)
    
    test_results = []
    class_names = [LABELS[i]['name'] for i in sorted(LABELS.keys())]
    
    for idx, row in results_df.iterrows():
        model_name = row['Model']
        checkpoint = row['Checkpoint']
        
        logging.info(f"\nTesting {model_name}...")
        
        # Load model
        if model_name == 'ConvNeXt_Tiny':
            model = build_convnext(len(LABELS))
        elif model_name == 'EfficientNetV2_S':
            model = build_efficientnetv2(len(LABELS))
        elif model_name == 'MobileNetV2':
            model = build_mobilenet(len(LABELS))
        else:
            model = build_efficientnet(len(LABELS))
        
        model.load_state_dict(torch.load(checkpoint, map_location=DEVICE))
        model = model.to(DEVICE)
        metrics = evaluate_model(model, test_loader, DEVICE)
        
        test_results.append({
            'Model': model_name,
            'Test_Accuracy': metrics['accuracy'],
            'Test_Precision': metrics['precision'],
            'Test_Recall': metrics['recall'],
            'Test_F1': metrics['f1']
        })
        
        logging.info(f"  Accuracy: {metrics['accuracy']:.4f}")
        logging.info(f"  F1: {metrics['f1']:.4f}")
        
        # Per-class analysis
        analyze_per_class_performance(metrics, class_names, save_dir)
        
        # Confidence calibration
        analyze_confidence_calibration(metrics, save_dir)
        
        # Save confusion matrix
        plt.figure(figsize=(8, 6))
        sns.heatmap(metrics['confusion_matrix'], annot=True, fmt='d', cmap='Blues',
                   xticklabels=class_names, yticklabels=class_names)
        plt.title(f'{model_name} - Test Set Confusion Matrix')
        plt.xlabel('Predicted')
        plt.ylabel('True')
        plt.tight_layout()
        
        cm_path = os.path.join(save_dir, f'{model_name}_test_cm.png')
        plt.savefig(cm_path, dpi=150, bbox_inches='tight')
        plt.close()
        
        # Classification report
        report = classification_report(metrics['y_true'], metrics['y_pred'], 
                                      target_names=class_names, digits=4)
        
        report_path = os.path.join(save_dir, f'{model_name}_test_report.txt')
        with open(report_path, 'w') as f:
            f.write(f"Model: {model_name}\n")
            f.write(f"Test Accuracy: {metrics['accuracy']:.4f}\n\n")
            f.write("Classification Report:\n")
            f.write(report)
        
        del model
        torch.cuda.empty_cache()
        gc.collect()
    
    test_results_df = pd.DataFrame(test_results)
    test_csv = os.path.join(save_dir, 'test_results.csv')
    test_results_df.to_csv(test_csv, index=False)
    
    logging.info(f"\n✓ Test results saved: {test_csv}")
    return test_results_df

if results_df is not None and labeled_crops_df is not None:
    test_results_df = evaluate_on_test_set(results_df, labeled_crops_df, OUTPUT_DIRS["test_results"])

# %% [markdown]
# ## PHASE 11: Visualizations

# %%
def plot_training_curves(histories, save_dir):
    if not histories:
        return
    
    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle('Training Curves - Enhanced Models', fontsize=16, fontweight='bold')
    
    ax = axes[0, 0]
    for model_name, history in histories.items():
        epochs = range(1, len(history['train_loss']) + 1)
        ax.plot(epochs, history['train_loss'], label=f"{model_name} Train", linewidth=2)
        ax.plot(epochs, history['val_loss'], label=f"{model_name} Val", linewidth=2, linestyle='--')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('Loss')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    ax = axes[0, 1]
    for model_name, history in histories.items():
        epochs = range(1, len(history['train_acc']) + 1)
        ax.plot(epochs, history['train_acc'], label=f"{model_name} Train", linewidth=2)
        ax.plot(epochs, history['val_acc'], label=f"{model_name} Val", linewidth=2, linestyle='--')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Accuracy')
    ax.set_title('Accuracy')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    ax = axes[1, 0]
    for model_name, history in histories.items():
        epochs = range(1, len(history['val_loss']) + 1)
        ax.plot(epochs, history['val_loss'], label=model_name, linewidth=2, marker='o', markersize=4)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Validation Loss')
    ax.set_title('Val Loss Comparison')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    ax = axes[1, 1]
    for model_name, history in histories.items():
        epochs = range(1, len(history['val_acc']) + 1)
        ax.plot(epochs, history['val_acc'], label=model_name, linewidth=2, marker='o', markersize=4)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Validation Accuracy')
    ax.set_title('Val Accuracy Comparison')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'training_curves.png'), dpi=150, bbox_inches='tight')
    plt.show()

def plot_model_comparison(results_df, test_results_df, save_dir):
    if results_df is None:
        return
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Enhanced Model Performance', fontsize=16, fontweight='bold')
    
    models = results_df['Model'].values
    
    # Metrics comparison
    ax = axes[0, 0]
    if test_results_df is not None:
        merged = results_df.merge(test_results_df, on='Model')
        x = np.arange(len(models))
        width = 0.35
        
        ax.bar(x - width/2, merged['Accuracy'], width, label='Val Acc', alpha=0.8)
        ax.bar(x + width/2, merged['Test_Accuracy'], width, label='Test Acc', alpha=0.8)
        
        ax.set_xlabel('Model')
        ax.set_ylabel('Accuracy')
        ax.set_title('Validation vs Test Accuracy')
        ax.set_xticks(x)
        ax.set_xticklabels(models, rotation=45, ha='right')
        ax.legend()
        ax.grid(True, alpha=0.3, axis='y')
    
    # Size vs Accuracy
    ax = axes[0, 1]
    ax.scatter(results_df['Params (M)'], results_df['Accuracy'], s=200, alpha=0.6)
    for idx, row in results_df.iterrows():
        ax.annotate(row['Model'], (row['Params (M)'], row['Accuracy']), 
                   xytext=(5, 5), textcoords='offset points')
    ax.set_xlabel('Model Size (M)')
    ax.set_ylabel('Accuracy')
    ax.set_title('Size vs Accuracy')
    ax.grid(True, alpha=0.3)
    
    # Speed
    ax = axes[1, 0]
    ax.barh(models, results_df['FPS'].values, color='steelblue')
    ax.set_xlabel('FPS')
    ax.set_title('Inference Speed')
    ax.grid(True, alpha=0.3, axis='x')
    
    # Overall
    ax = axes[1, 1]
    metrics = ['Accuracy', 'Precision', 'Recall', 'F1']
    x = np.arange(len(models))
    width = 0.2
    
    for i, metric in enumerate(metrics):
        values = results_df[metric].values
        ax.bar(x + i*width, values, width, label=metric)
    
    ax.set_xlabel('Model')
    ax.set_ylabel('Score')
    ax.set_title('All Metrics')
    ax.set_xticks(x + width * 1.5)
    ax.set_xticklabels(models, rotation=45, ha='right')
    ax.legend()
    ax.grid(True, alpha=0.3, axis='y')
    ax.set_ylim(0, 1.1)
    
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'model_comparison.png'), dpi=150, bbox_inches='tight')
    plt.show()

if histories and results_df is not None:
    plot_training_curves(histories, OUTPUT_DIRS["training_curves"])
    plot_model_comparison(results_df, test_results_df if 'test_results_df' in locals() else None, OUTPUT_DIRS["comparison"])

# %% [markdown]
# ## PHASE 12: Export Models

# %%
def export_for_production(model, model_name, save_dir, example_input):
    """Export optimized models for deployment"""
    model.eval()
    
    # 1. TorchScript
    try:
        traced = torch.jit.trace(model, example_input)
        traced_path = os.path.join(save_dir, f'{model_name}_traced.pt')
        torch.jit.save(traced, traced_path)
        logging.info(f"✓ Exported TorchScript: {traced_path}")
    except Exception as e:
        logging.warning(f"TorchScript export failed: {e}")
    
    # 2. ONNX
    try:
        onnx_path = os.path.join(save_dir, f'{model_name}.onnx')
        torch.onnx.export(
            model, example_input, onnx_path,
            input_names=['input'], output_names=['output'],
            dynamic_axes={'input': {0: 'batch_size'}, 'output': {0: 'batch_size'}},
            opset_version=11
        )
        logging.info(f"✓ Exported ONNX: {onnx_path}")
    except Exception as e:
        logging.warning(f"ONNX export failed: {e}")

# %% [markdown]
# ## PHASE 13: Select Best & Demo

# %%
def select_best_model(results_df, test_results_df=None):
    """Select best model based on test F1 or val F1"""
    if test_results_df is not None:
        merged = results_df.merge(test_results_df, on='Model')
        best_idx = merged['Test_F1'].idxmax()
        best_model = merged.loc[best_idx, 'Model']
        best_score = merged.loc[best_idx, 'Test_F1']
        score_type = "Test F1"
    else:
        best_idx = results_df['F1'].idxmax()
        best_model = results_df.loc[best_idx, 'Model']
        best_score = results_df.loc[best_idx, 'F1']
        score_type = "Val F1"
    
    best_checkpoint = results_df.loc[results_df['Model'] == best_model, 'Checkpoint'].values[0]
    
    logging.info("\n" + "=" * 60)
    logging.info("BEST MODEL SELECTION")
    logging.info("=" * 60)
    logging.info(f"Best Model: {best_model}")
    logging.info(f"{score_type}: {best_score:.4f}")
    logging.info(f"Checkpoint: {best_checkpoint}")
    
    # Export best model
    if best_model == 'ConvNeXt_Tiny':
        model = build_convnext(len(LABELS))
    elif best_model == 'EfficientNetV2_S':
        model = build_efficientnetv2(len(LABELS))
    elif best_model == 'MobileNetV2':
        model = build_mobilenet(len(LABELS))
    else:
        model = build_efficientnet(len(LABELS))
    
    model.load_state_dict(torch.load(best_checkpoint, map_location=DEVICE))
    model = model.to(DEVICE)
    
    example_input = torch.randn(1, 3, 224, 224).to(DEVICE)
    export_for_production(model, best_model, OUTPUT_DIRS["exports"], example_input)
    
    return best_model, best_checkpoint

if results_df is not None:
    best_model_name, best_model_path = select_best_model(
        results_df, 
        test_results_df if 'test_results_df' in locals() else None
    )

# %% [markdown]
# ## PHASE 14: Inference Class with TTA

# %%
class EnhancedRiceFieldAnalyzer:
    """Enhanced inference pipeline with TTA"""
    
    def __init__(self, yolo_model_path, classification_model_path, model_name, device=DEVICE):
        self.device = device
        
        try:
            self.yolo_model = YOLO(yolo_model_path)
        except Exception as e:
            raise RuntimeError(f"Failed to load YOLO model: {e}")
        
        # Load classification model
        if model_name == 'ConvNeXt_Tiny':
            self.classifier = build_convnext(len(LABELS))
        elif model_name == 'EfficientNetV2_S':
            self.classifier = build_efficientnetv2(len(LABELS))
        elif model_name == 'MobileNetV2':
            self.classifier = build_mobilenet(len(LABELS))
        else:
            self.classifier = build_efficientnet(len(LABELS))
        
        try:
            self.classifier.load_state_dict(torch.load(classification_model_path, map_location=device))
            self.classifier.to(device)
            self.classifier.eval()
        except Exception as e:
            raise RuntimeError(f"Failed to load classification model: {e}")
        
        self.transform = val_transform
        self.class_names = [LABELS[i]['name'] for i in sorted(LABELS.keys())]
    
    @torch.no_grad()
    def analyze_field_image(self, image_path, yolo_conf=0.3, use_tta=False):
        """Analyze single field image with optional TTA"""
        # Validate input
        if not os.path.exists(image_path):
            raise FileNotFoundError(f"Image not found: {image_path}")
        
        try:
            # YOLO detection
            yolo_results = self.yolo_model.predict(image_path, conf=yolo_conf, verbose=False)
            
            img = cv2.imread(image_path)
            if img is None:
                raise ValueError(f"Cannot read image: {image_path}")
            img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            
            predictions = []
            crops_for_viz = []
            
            for result in yolo_results[0].boxes:
                x1, y1, x2, y2 = map(int, result.xyxy[0].cpu().numpy())
                crop = img_rgb[y1:y2, x1:x2]
                
                if crop.size == 0 or crop.shape[0] < 30 or crop.shape[1] < 30:
                    continue
                
                # Classify with optional TTA
                crop_pil = Image.fromarray(crop)
                
                if use_tta:
                    # Test-Time Augmentation
                    probs_list = []
                    for _ in range(5):
                        crop_tensor = self.transform(crop_pil).unsqueeze(0).to(self.device)
                        output = self.classifier(crop_tensor)
                        probs_list.append(F.softmax(output, dim=1))
                    
                    probs = torch.stack(probs_list).mean(0)
                    conf, pred = torch.max(probs, 1)
                else:
                    crop_tensor = self.transform(crop_pil).unsqueeze(0).to(self.device)
                    output = self.classifier(crop_tensor)
                    probs = F.softmax(output, dim=1)
                    conf, pred = torch.max(probs, 1)
                
                pred_class = self.class_names[pred.item()]
                conf_score = conf.item()
                
                predictions.append({
                    'class': pred_class,
                    'confidence': conf_score,
                    'bbox': (x1, y1, x2, y2)
                })
                
                crops_for_viz.append((crop, pred_class, conf_score))
            
            return predictions, crops_for_viz, img_rgb
        
        except Exception as e:
            logging.error(f"Analysis failed for {image_path}: {e}")
            return [], [], None
    
    def visualize_analysis(self, image_path, predictions, crops, img_rgb, save_path=None):
        """Visualize field analysis results"""
        if img_rgb is None or len(predictions) == 0:
            logging.warning("No predictions to visualize")
            return {}
        
        n_crops = min(5, len(crops))
        fig = plt.figure(figsize=(16, 10))
        
        # Main image with bboxes
        ax_main = plt.subplot(2, 3, (1, 4))
        img_draw = img_rgb.copy()
        
        for pred in predictions:
            x1, y1, x2, y2 = pred['bbox']
            color = (0, 255, 0) if pred['class'] == 'healthy' else (255, 0, 0)
            cv2.rectangle(img_draw, (x1, y1), (x2, y2), color, 3)
            
            label_text = f"{pred['class']}: {pred['confidence']:.2f}"
            cv2.putText(img_draw, label_text, (x1, y1-10), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
        
        ax_main.imshow(img_draw)
        ax_main.set_title(f'Field Analysis: {len(predictions)} leaves detected', 
                         fontsize=14, fontweight='bold')
        ax_main.axis('off')
        
        # Sample crops
        for i in range(n_crops):
            ax = plt.subplot(2, 3, i+2)
            crop, cls, conf = crops[i]
            ax.imshow(crop)
            ax.set_title(f'{cls}\nConf: {conf:.3f}', fontsize=10)
            ax.axis('off')
        
        # Disease distribution
        ax_dist = plt.subplot(2, 3, 6)
        disease_counts = {}
        for p in predictions:
            disease_counts[p['class']] = disease_counts.get(p['class'], 0) + 1
        
        colors_map = {
            'healthy': 'green',
            'brown_spot': 'brown',
            'leaf_blast': 'red',
            'leaf_blight': 'orange'
        }
        
        diseases = list(disease_counts.keys())
        counts = list(disease_counts.values())
        colors = [colors_map.get(d, 'gray') for d in diseases]
        
        ax_dist.pie(counts, labels=diseases, autopct='%1.1f%%', colors=colors)
        ax_dist.set_title('Disease Distribution', fontsize=12, fontweight='bold')
        
        plt.tight_layout()
        
        if save_path:
            plt.savefig(save_path, dpi=150, bbox_inches='tight')
        
        plt.show()
        
        # Print summary
        print("\n" + "="*60)
        print("FIELD ANALYSIS SUMMARY")
        print("="*60)
        print(f"Image: {os.path.basename(image_path)}")
        print(f"Total leaves detected: {len(predictions)}")
        print("\nDisease Distribution:")
        for disease, count in sorted(disease_counts.items()):
            pct = count/len(predictions)*100
            print(f"  {disease:15s}: {count:3d} ({pct:5.1f}%)")
        
        healthy_pct = disease_counts.get('healthy', 0) / len(predictions) * 100 if len(predictions) > 0 else 0
        
        if healthy_pct >= 80:
            health_status = "EXCELLENT"
        elif healthy_pct >= 60:
            health_status = "GOOD"
        elif healthy_pct >= 40:
            health_status = "FAIR"
        else:
            health_status = "POOR"
        
        print(f"\nField Health Status: {health_status}")
        print("="*60)
        
        return disease_counts

# Initialize analyzer
if 'best_model_path' in locals() and 'best_yolo_model' in locals():
    analyzer = EnhancedRiceFieldAnalyzer(
        yolo_model_path=best_yolo_model,
        classification_model_path=best_model_path,
        model_name=best_model_name,
        device=DEVICE
    )
    
    logging.info("\n✓ Enhanced Analyzer initialized and ready for inference")

# %% [markdown]
# ## PHASE 15: Demo

# %%
def demo_on_test_images(analyzer, field_data, n_samples=3, save_dir=None):
    """Demo inference on test images"""
    logging.info("\n" + "=" * 60)
    logging.info("DEMO INFERENCE ON TEST IMAGES")
    logging.info("=" * 60)
    
    test_images = field_data['test']
    
    if len(test_images) == 0:
        logging.warning("No test images available")
        return
    
    samples = random.sample(test_images, min(n_samples, len(test_images)))
    
    for i, sample in enumerate(samples):
        image_path = sample['image_path']
        true_label = sample['true_label']
        
        logging.info(f"\n--- Test Image {i+1}/{len(samples)} ---")
        logging.info(f"True label: {true_label}")
        logging.info(f"Path: {image_path}")
        
        predictions, crops, img_rgb = analyzer.analyze_field_image(image_path, yolo_conf=0.3, use_tta=False)
        
        if save_dir:
            save_path = os.path.join(save_dir, f"demo_test_{i+1}.png")
        else:
            save_path = None
        
        disease_dist = analyzer.visualize_analysis(
            image_path, 
            predictions, 
            crops, 
            img_rgb, 
            save_path=save_path
        )

if 'analyzer' in locals() and 'field_data' in locals():
    demo_on_test_images(analyzer, field_data, n_samples=3, save_dir=OUTPUT_DIRS["demo_outputs"])

# %% [markdown]
# ## PHASE 16: Final Summary

# %%
def export_final_summary(output_dir):
    """Export comprehensive summary"""
    summary = {
        'pipeline': 'Enhanced YOLO-First Pipeline v2',
        'enhancements': [
            'ConvNeXt & EfficientNetV2 models',
            'MixUp augmentation',
            'Label smoothing',
            'Focal loss ready',
            'Per-class analysis',
            'Confidence calibration',
            'Model export (TorchScript, ONNX)',
            'Enhanced error handling'
        ],
        'timestamp': datetime.now().isoformat(),
        'device': str(DEVICE),
        'cuda_available': CUDA_AVAILABLE,
        'phases_completed': []
    }
    
    if 'collected_df' in globals():
        summary['data_collection'] = {
            'total_images': len(collected_df),
            'sources': len(DATASET_SOURCES),
            'labels': len(LABELS)
        }
        summary['phases_completed'].append('Data Collection')
    
    if 'field_data' in globals():
        summary['dataset_splits'] = {
            'train': len(field_data['train']),
            'val': len(field_data['val']),
            'test': len(field_data['test'])
        }
        summary['phases_completed'].append('Dataset Preparation')
    
    if 'best_yolo_model' in globals():
        summary['yolo_training'] = {
            'model': 'YOLOv8n',
            'epochs': 30,
            'checkpoint': best_yolo_model
        }
        summary['phases_completed'].append('YOLO Training')
    
    if 'crops_df' in globals():
        summary['crop_extraction'] = {
            'total_crops': len(crops_df),
            'train': len(crops_df[crops_df['split']=='train']),
            'val': len(crops_df[crops_df['split']=='val']),
            'test': len(crops_df[crops_df['split']=='test'])
        }
        summary['phases_completed'].append('Crop Extraction')
    
    if 'results_df' in globals():
        summary['classification_training'] = {
            'models_trained': len(results_df),
            'epochs': 25,
            'models': results_df['Model'].tolist()
        }
        summary['phases_completed'].append('Classification Training')
    
    if 'test_results_df' in globals():
        summary['test_evaluation'] = test_results_df.to_dict('records')
        summary['phases_completed'].append('Test Evaluation')
    
    if 'best_model_name' in globals():
        summary['best_model'] = {
            'name': best_model_name,
            'checkpoint': best_model_path,
            'val_f1': float(results_df[results_df['Model']==best_model_name]['F1'].values[0])
        }
        
        if 'test_results_df' in globals():
            summary['best_model']['test_f1'] = float(
                test_results_df[test_results_df['Model']==best_model_name]['Test_F1'].values[0]
            )
    
    import json
    summary_path = os.path.join(output_dir, 'pipeline_summary.json')
    with open(summary_path, 'w') as f:
        json.dump(summary, f, indent=2)
    
    logging.info(f"\n✓ Pipeline summary: {summary_path}")
    return summary

summary = export_final_summary(OUTPUT_DIRS["results"])

# Final report
logging.info("\n" + "=" * 80)
logging.info("ENHANCED PIPELINE COMPLETE")
logging.info("=" * 80)
logging.info(f"Output: {PATH_OUTPUT}")
logging.info(f"\nPhases completed: {len(summary['phases_completed'])}")
for phase in summary['phases_completed']:
    logging.info(f"  ✓ {phase}")

logging.info(f"\nEnhancements applied:")
for enhancement in summary['enhancements']:
    logging.info(f"  • {enhancement}")

if 'best_model' in summary:
    logging.info(f"\nBest Model: {summary['best_model']['name']}")
    logging.info(f"Val F1: {summary['best_model']['val_f1']:.4f}")
    if 'test_f1' in summary['best_model']:
        logging.info(f"Test F1: {summary['best_model']['test_f1']:.4f}")

logging.info("\n" + "=" * 80)
logging.info("Ready for production deployment!")
logging.info(f"Exported models: {OUTPUT_DIRS['exports']}")
logging.info("=" * 80)

# %%
# Example usage:
"""
# Analyze your own image:
image_path = "path/to/your/field_image.jpg"
predictions, crops, img_rgb = analyzer.analyze_field_image(image_path, use_tta=True)
analyzer.visualize_analysis(image_path, predictions, crops, img_rgb)
"""