# %% [markdown]
# # YOLO-First Rice Disease Classification - Production Ready
# Complete pipeline with test set and inference demo
...
PATH_OUTPUT = get_output_folder("../output", "yolo_complete_v1")
...
@torch.no_grad()
def measure_latency(model, device, input_size=(1, 3, 384, 384), runs=50):
    model.eval()
...
if labeled_crops_df is not None and len(labeled_crops_df) > 0:
    ...
    for model_name, model_builder in models_to_train.items():
        ...
        logging.info(f"{model_name}: Acc={metrics['accuracy']:.4f} F1={metrics['f1']:.4f} FPS={fps:.1f}")
...
# %% [markdown]
# ## PHASE 11: Inference & Demo Functions
...
if 'best_model_path' in locals() and 'best_yolo_model' in locals():
    ...
    logging.info("\n✓ Analyzer initialized and ready for inference")

# %% [markdown]
# ## PHASE 12: Demo on Test Images
...
def demo_on_test_images(analyzer, field_data, n_samples=3, save_dir=None):
    ...
    for i, sample in enumerate(samples):
        ...
        predictions, crops, img_rgb = analyzer.analyze_field_image(image_path, yolo_conf=0.3)
...
# %% [markdown]
# ## PHASE 13: Final Summary & Export

# %%
def export_final_summary(output_dir):
    """Export comprehensive summary"""
    summary = {
        'pipeline': 'YOLO-First Complete Pipeline',
...
logging.info("\n" + "=" * 80)
logging.info("Ready for production inference!")
logging.info(f"Use: analyzer.analyze_field_image('path/to/image.jpg')")
logging.info("=" * 80)

# %%
# Example: Analyze custom image
"""
# To analyze your own image:
image_path = "path/to/your/field_image.jpg"
predictions, crops, img_rgb = analyzer.analyze_field_image(image_path)
