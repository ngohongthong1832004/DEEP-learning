# %% [markdown]
# # Complete Lightweight Rice Disease Detection Pipeline
# Mobile/Drone-Optimized: Lesion Detection → MobileNetV3-Small Classification
# WITH ALL PRODUCTION REFINEMENTS
...
# %%
# ===== LIGHTWEIGHT CONFIGURATION WITH ALL REFINEMENTS =====
CONFIG = {
    ...
    # Data loader - REFINED: prefetch_factor added
    'num_workers': 4,
    'pin_memory': True,
    ...
    # YOLO (LESION DETECTION - ENHANCED!)
    # 'yolo_epochs': 3,
    'yolo_epochs': 30,
    'yolo_imgsz': 768,  # REFINED: Higher for small lesions
    'yolo_batch': 16,
    'yolo_conf': 0.25,
    'yolo_mosaic': 0.5,  # REFINED: Added
    'yolo_mixup': 0.1,   # REFINED: Added
    'yolo_close_mosaic': 10,  # REFINED: Added
    
    # Splits
    'val_size': 0.15,
    'test_size': 0.15,
    
    # Lesion-based Filtering - REFINED!
    'crops_per_image': 3,
    'min_crop_size': 64,
    'min_lesion_area_ratio': 0.001,  # REFINED: Relative to LEAF (not image)
    'max_lesion_area_ratio': 0.15,   # REFINED: Relative to LEAF
    'min_circularity': 0.15,         # REFINED: More permissive
    'min_aspect_ratio': 0.2,
    'max_aspect_ratio': 5.0,
    
    # NMS & Fallback - NEW PRODUCTION FEATURES!
    'crop_nms_iou': 0.5,
    'enable_fallback_roi': True,
    'fallback_roi_count': 2,
    
    # Quantization - ENHANCED!
    'enable_quantization': True,
    'enable_onnx_quantization': True,
}
...
PATH_OUTPUT = get_output_folder("../output", "GK-lesion-COMPLETE")
...
# %% [markdown]
# ## PHASE 2: REFINED Pseudo-Label LESION Detection

# %%
def create_pseudo_lesion_bboxes(img_path, class_id=0):
    """
    PRODUCTION-REFINED: Correct circularity + leaf-relative area
    
    Improvements:
    1. Use actual contour perimeter (not bbox perimeter) for circularity
    2. Use lesion area relative to LEAF area (not total image)
    3. Better morphology operations
    """
    try:
        ...
        # 2) Lesion candidates: brown OR dark spots
        ...
        # 3) Find contours - REFINED: use contours directly
        ...
        for cnt in contours:
            ...
            # REFINED: dùng tỷ lệ theo vùng lá
            ...
            # REFINED: Use actual contour perimeter for circularity
...
def prepare_lesion_yolo_dataset(df, output_dir, val_size=0.15, test_size=0.15):
    ...
    logging.info("\n" + "="*60)
    logging.info("DATASET PREPARATION FOR YOLO (1-CLASS LESION - REFINED)")
    logging.info("="*60)
    ...
    for split, split_df in [('train', train_df), ('val', val_df), ('test', test_df)]:
        ...
        for idx, row in tqdm(split_df.iterrows(), total=len(split_df), desc=f"Preparing {split}"):
            ...
            try:
                ...
                # Create pseudo lesion labels with REFINED method
...
# %% [markdown]
# ## PHASE 3: ENHANCED YOLO Training (Lesion Detection)

# %%
def train_yolo_lesion_detector(yaml_path, output_dir, epochs=30, imgsz=768, batch=16):
    if not YOLO_AVAILABLE:
        logging.error("YOLO not available")
        return None
    
    logging.info("\n" + "="*60)
    logging.info("YOLO TRAINING (LESION DETECTION - ENHANCED)")
    logging.info("="*60)
    
    yolo_device = '0' if torch.cuda.is_available() else 'cpu'
    model = YOLO('yolov8n.pt')  # Nano model - very lightweight!
    
    # REFINED: Enhanced hyperparameters for small lesion detection
    results = model.train(
        ...
        # REFINED: augment gọn cho lesion nhỏ:
        mosaic=CONFIG['yolo_mosaic'], mixup=CONFIG['yolo_mixup'],
        hsv_h=0.015, hsv_s=0.7, hsv_v=0.4,
        scale=0.5, shear=0.0, perspective=0.0,
        close_mosaic=CONFIG['yolo_close_mosaic'],  # 10 epoch cuối tắt mosaic
...
# %% [markdown]
# ## PHASE 4: PRODUCTION Extract Lesion Crops with NMS + Fallback
...
def nms_xyxy(boxes, scores, iou_thr=0.5):
    """
    PRODUCTION: NMS for crop-level deduplication
    """
    if len(boxes) == 0:
        return []
...
def fallback_lesion_rois(img_rgb, K=2):
    """
    PRODUCTION: Fallback ROI extraction when YOLO fails
    Used primarily for healthy images
    """
    try:
...
def visualize_lesion_crop_samples(model, field_data, output_dir, n_samples=6):
    """Visualize lesion crop extraction with NMS"""
    logging.info("\n" + "="*60)
    ...
    for sample_idx, item in enumerate(train_samples):
        ...
        try:
            ...
            # Apply NMS
            if len(boxes_xyxy) > 0:
                keep_indices = nms_xyxy(boxes_xyxy, confidences, CONFIG['crop_nms_iou'])
            else:
                keep_indices = []
            ...
            plt.suptitle(f'Sample {sample_idx+1}: Refined Lesion Extraction (with NMS)', 
...
def extract_lesion_crops(yolo_model_path, field_data, output_dir, confidence=0.25):
    """
    PRODUCTION: Extract lesion crops with NMS and fallback
    - Diseased images: keep top-K high-score lesions
    - Healthy images: keep top-K low-score regions
    """
    logging.info("\n" + "="*60)
    logging.info("EXTRACTING LESION CROPS (PRODUCTION: NMS + FALLBACK)")
    logging.info("="*60)
    logging.info(f"Strategy: Keep top-{CONFIG['crops_per_image']} lesions per image")
    logging.info(f"NMS IoU threshold: {CONFIG['crop_nms_iou']}")
    logging.info(f"Fallback ROI: {CONFIG['enable_fallback_roi']}")
    ...
    for split in ['train', 'val', 'test']:
        ...
        for item in tqdm(field_data[split], desc=f"Extracting {split}"):
            ...
            try:
                ...
                # Collect boxes and confidences for NMS
                boxes_xyxy = []
                confidences = []
                ...
                # Collect proposals with lesion scores
                proposals = []
                for idx in keep_indices:
                    x1, y1, x2, y2 = boxes_xyxy[idx]
                ...
                # PRODUCTION: Fallback if no detections
                if len(proposals) == 0 and CONFIG['enable_fallback_roi']:
                    proposals = fallback_lesion_rois(img_rgb, K=CONFIG['fallback_roi_count'])
                    if len(proposals) > 0:
                        fallback_count += 1
    ...
    logging.info(f"✓ Fallback ROI used: {fallback_count} times")
    for split in ['train', 'val', 'test']:
...
def create_data_analysis_report(collected_df, crops_df, output_dir):
    ...
    ax2.set_title('Refined Lesion Crops per Class', fontsize=11, fontweight='bold')
    ...
    ax7.set_title('Refined Lesions per Parent Image', fontsize=11, fontweight='bold')
    ax7.legend()
    ...
    ax9.axis('off')
    stats_text = f"""
    DATA SUMMARY (PRODUCTION PIPELINE)
    ═══════════════════════════════════════
    ...
    PRODUCTION Features:
      ✓ Lesion detection (1-class)
      ✓ Contour circularity
      ✓ Leaf-relative area
      ✓ NMS (IoU={CONFIG['crop_nms_iou']})
      ✓ Fallback ROI
      ✓ MobileNetV3 (~2.5M)
      ✓ Mobile/Drone ready
    """
    ...
    plt.suptitle('Comprehensive Data Analysis Report (Production Pipeline)', fontsize=16, fontweight='bold', y=0.995)
    plt.tight_layout()
...
class SymmetricCrossEntropy(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        rce = (-torch.sum(pred * torch.log(y + 1e-7), dim=1)).mean()
...
# ===== DATA LOADERS (REFINED with prefetch_factor) =====
...
def train_production_model(model, train_df, val_df, epochs):
    ...
    scheduler = build_warmup_cosine_scheduler(optimizer, warmup_epochs=CONFIG['warmup_epochs'], total_epochs=epochs)
    ...
    best_path = os.path.join(OUTPUT_DIRS["weights"], "best_production_classifier.pth")
    
    current_size = CONFIG['img_size']
    
    logging.info(f"\nTraining PRODUCTION MobileNetV3-Small...")
...
history, best_checkpoint = train_production_model(model, train_crops, val_crops, CONFIG['epochs'])
...
create_training_report(history, "MobileNetV3-Production", OUTPUT_DIRS["plots"])
...
def create_test_results_report(test_metrics, class_names, output_dir):
    ...
    plt.suptitle('Test Results Report (Production Pipeline)', fontsize=16, fontweight='bold', y=0.995)
    plt.tight_layout()
...
with open(report_path, 'w') as f:
    f.write(f"Production MobileNetV3-Small (Lesion-First) Test Results\n")
    f.write("="*60 + "\n\n")
    f.write(f"Test Accuracy: {test_metrics['accuracy']:.4f}\n\n")
    f.write("Production Optimizations:\n")
    f.write(f"  Model: MobileNetV3-Small (~2.5M params)\n")
    f.write(f"  YOLO: 1-class lesion detection (enhanced)\n")
    f.write(f"  Pseudo-labels: Contour circularity + Leaf-relative area\n")
    f.write(f"  NMS: IoU={CONFIG['crop_nms_iou']}\n")
    f.write(f"  Fallback ROI: {CONFIG['enable_fallback_roi']}\n")
    ...
    f.write(f"  Warmup + Cosine LR: {CONFIG['warmup_epochs']} epochs\n")
    f.write(f"  prefetch_factor: {CONFIG['prefetch_factor']}\n")
    f.write(f"  Mobile/Drone Ready: YES\n\n")
    f.write("Classification Report:\n")
    f.write(report)

# %% [markdown]
# ## PHASE 11: PRODUCTION Export & Quantization
...
def export_for_production(model, model_name, save_dir, example_input):
    ...
    try:
        ...
        # PRODUCTION: ONNX INT8 Quantization
        if CONFIG['enable_onnx_quantization']:
            try:
                ...
                logging.info(f"✓ ONNX INT8: {onnx_int8_path}")
                
                # Check size reduction
                import os as os_module
                orig_size = os_module.path.getsize(onnx_path) / 1024**2
                quant_size = os_module.path.getsize(onnx_int8_path) / 1024**2
                logging.info(f"  Size: {orig_size:.2f}MB -> {quant_size:.2f}MB ({quant_size/orig_size*100:.1f}%)")
            except Exception as e:
                logging.warning(f"ONNX quantization failed: {e}")
    except Exception as e:
    ...
    if CONFIG['enable_quantization']:
        try:
            ...
            logging.info(f"✓ PyTorch INT8: {quant_path}")
        except Exception as e:
            logging.warning(f"PyTorch quantization failed: {e}")

example_input = torch.randn(1, 3, CONFIG['img_size'], CONFIG['img_size']).to(DEVICE)
export_for_production(model, "MobileNetV3_Production", OUTPUT_DIRS["exports"], example_input)

# %% [markdown]
# ## PHASE 12: PRODUCTION Analyzer with Bag-Level Aggregation

# %%
class ProductionRiceAnalyzer:
    """
    PRODUCTION Analyzer with Bag-Level Aggregation
    
    Complete pipeline:
    1. Detect lesions using YOLO
    2. Apply NMS and fallback ROI
    3. Extract and classify crops
    4. Aggregate predictions at bag level
    """
    ...
    def __init__(self, yolo_model_path, classification_model_path, device=DEVICE):
        ...
        # Load priors for logit adjustment
        self.priors = None
        if CONFIG['use_logit_adjustment']:
            train_df = crops_df[crops_df['split'] == 'train']
            num_classes = len(LABELS)
            self.priors = compute_priors(train_df, num_classes)
            logging.info(f"Analyzer initialized with Logit Adjustment Priors: {self.priors.tolist()}")
    
    def detect_lesions(self, image):
        """Detect lesions using YOLO"""
    ...
    def filter_and_select_boxes(self, boxes, image_width, image_height):
        """
        Apply NMS and select top-K boxes
        Apply fallback ROI if no detections
        """
        if not boxes:
            # Fallback ROI
            if CONFIG['enable_fallback_roi']:
                x_center, y_center = image_width // 2, image_height // 2
                x1 = max(0, x_center - image_width // 4)
                y1 = max(0, y_center - image_height // 4)
                x2 = min(image_width, x_center + image_width // 4)
                y2 = min(image_height, y_center + image_height // 4)
                boxes = [(x1, y1, x2, y2, 0.5)]
                logging.info("Applied fallback ROI")
            else:
                return []
        ...
        while order.size > 0:
            ...
            inds = np.where(ovr <= CONFIG['crop_nms_iou'])[0]
            order = order[inds + 1]
        
        filtered_boxes = [boxes[k] for k in keep]
        
        # Select top-K by confidence
        filtered_boxes.sort(key=lambda x: x[4], reverse=True)
        selected_boxes = filtered_boxes[:CONFIG['crops_per_image']]
        
        return selected_boxes
    
    def classify_crops(self, image, boxes):
        """Extract crops and classify them"""
        crops = []
        valid_boxes = []
        ...
        for (x1, y1, x2, y2, conf) in boxes:
            ...
            # Check minimum size
            if crop.width < CONFIG['min_crop_size'] or crop.height < CONFIG['min_crop_size']:
                continue
        ...
        with torch.no_grad():
            logits = self.classifier(crops_tensor)
            
            # Apply logit adjustment if enabled
            if CONFIG['use_logit_adjustment'] and self.priors is not None:
                logits = apply_logit_adjustment(logits, self.priors, CONFIG['logit_adjustment_tau'])
            
            probs = F.softmax(logits, dim=1)
            preds = probs.argmax(1).cpu().numpy()
        
        return preds.tolist(), probs.cpu().numpy(), valid_boxes
    ...
    def analyze_image(self, image_path):
        ...
        Returns:
            dict with:
                ...
                - crop_preds: list of predicted class IDs
                - crop_probs: array of class probabilities
                - bag_pred: final predicted class ID (majority vote)
                - bag_probs: averaged class probabilities
        """
        # Load image
        image = Image.open(image_path).convert("RGB")
        
        # Detect lesions
        boxes = self.detect_lesions(image_path)
        
        # Filter and select boxes
        boxes = self.filter_and_select_boxes(boxes, image.width, image.height)
        
        if not boxes:
            logging.warning(f"No lesions detected in {image_path}")
            return None
        
        # Classify crops
        preds, probs, valid_boxes = self.classify_crops(image, boxes)
        
        if not preds:
            logging.warning(f"No valid crops in {image_path}")
            return None
        
        # Bag-level aggregation
        # Strategy 1: Majority voting
        bag_pred = np.bincount(preds, minlength=len(LABELS)).argmax()
        
        # Strategy 2: Average probabilities
        bag_probs = np.mean(probs, axis=0)
        ...
        result = {
            ...
            'bag_pred': bag_pred,
            'bag_probs': bag_probs,
            'num_lesions': len(valid_boxes)
        }
        
        return result
    
    def analyze_batch(self, image_paths, show_progress=True):
        """Analyze multiple images"""
        results = []
        
        iterator = tqdm(image_paths, desc="Analyzing images") if show_progress else image_paths
...
# Initialize analyzer
logging.info("\n" + "="*60)
logging.info("INITIALIZING PRODUCTION ANALYZER")
logging.info("="*60)

analyzer = ProductionRiceAnalyzer(
    yolo_model_path=best_yolo_model,
...
test_image_path = test_crops.iloc[0]['parent_image']
sample_result = analyzer.analyze_image(test_image_path)

if sample_result:
    logging.info(f"\nSample Analysis Result:")
    logging.info(f"  Image: {sample_result['image_path']}")
    logging.info(f"  Detected Lesions: {sample_result['num_lesions']}")
    logging.info(f"  Crop Predictions: {sample_result['crop_preds']}")
    logging.info(f"  Bag Prediction: {sample_result['bag_pred']} ({LABELS[sample_result['bag_pred']]['name']})")
    logging.info(f"  Bag Confidence: {sample_result['bag_probs'][sample_result['bag_pred']]:.3f}")

# %% [markdown]
# ## PHASE 13: Demo Visualization with Results

# %%
def create_demo_visualization(analyzer, test_df, output_dir, n_samples=6):
    """
    Create visual demo of the complete pipeline
    Shows: Original → Lesion Detection → Classification → Bag Prediction
    """
    logging.info("\n" + "="*60)
    logging.info("CREATING DEMO VISUALIZATION")
    logging.info("="*60)
    
    os.makedirs(output_dir, exist_ok=True)
    
    # Get unique parent images from test set
    parent_images = test_df['parent_image'].unique()
    samples = np.random.choice(parent_images, min(n_samples, len(parent_images)), replace=False)
    
    # Calculate grid layout
    n_rows = len(samples)
    fig = plt.figure(figsize=(20, 5 * n_rows))
    
    for idx, parent_path in enumerate(samples):
        try:
            # Get ground truth for this parent image
            parent_crops = test_df[test_df['parent_image'] == parent_path]
            true_label = parent_crops.iloc[0]['label_name']
            true_id = parent_crops.iloc[0]['label_id']
            
            # Analyze parent image
            result = analyzer.analyze_image(parent_path)
            ...
            # Draw boxes on image
            img_with_boxes = parent_img.copy()
            for (x1, y1, x2, y2, conf) in result['boxes']:
                cv2.rectangle(img_with_boxes, (x1, y1), (x2, y2), (255, 0, 0), 3)
                cv2.putText(img_with_boxes, f'{conf:.2f}', (x1, y1-10),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            
            # 1. Original image with detections
            ax1 = plt.subplot(n_rows, 3, idx*3 + 1)
            ax1.imshow(img_with_boxes)
            ax1.set_title(f'Parent Image (True: {true_label})\n{result["num_lesions"]} lesions detected',
                         fontsize=11, fontweight='bold')
            ax1.axis('off')
            
            # 2. Bag-level prediction
            bag_pred = result['bag_pred']
            bag_pred_name = LABELS[bag_pred]['name']
            bag_conf = result['bag_probs'][bag_pred]
            
            ax2 = plt.subplot(n_rows, 3, idx*3 + 2)
            
            # Bar chart of bag probabilities
            class_names = [LABELS[i]['name'] for i in sorted(LABELS.keys())]
            colors = ['green' if i == bag_pred else 'lightgray' for i in range(len(LABELS))]
            colors[true_id] = 'orange' if true_id != bag_pred else 'green'
            
            bars = ax2.barh(class_names, result['bag_probs'], color=colors)
            ax2.set_xlim(0, 1)
            ax2.set_xlabel('Probability', fontsize=10)
            ax2.set_title(f'Bag Prediction: {bag_pred_name}\nConf: {bag_conf:.3f}',
                         fontsize=11, fontweight='bold',
                         color='green' if bag_pred == true_id else 'red')
            ax2.grid(True, alpha=0.3, axis='x')
            
            # Add value labels on bars
            for bar, prob in zip(bars, result['bag_probs']):
                if prob > 0.05:
                    ax2.text(prob, bar.get_y() + bar.get_height()/2, f' {prob:.2f}',
                            va='center', fontsize=9)
            
            # 3. Crop-level predictions distribution
            ax3 = plt.subplot(n_rows, 3, idx*3 + 3)
            
            crop_preds = result['crop_preds']
            crop_pred_names = [LABELS[p]['name'] for p in crop_preds]
            
            if len(crop_pred_names) > 0:
                crop_counts = pd.Series(crop_pred_names).value_counts()
                
                # Pie chart
                colors_pie = plt.cm.Set3(range(len(crop_counts)))
                ...
                ax3.set_title(f'Crop Predictions\n({len(crop_preds)} crops analyzed)',
                             fontsize=11, fontweight='bold')
            
        except Exception as e:
            logging.warning(f"Error creating demo {idx}: {e}")
            continue
    
    plt.suptitle('Production Pipeline Demo: Lesion Detection → Classification → Bag Aggregation',
                fontsize=16, fontweight='bold', y=0.998)
    plt.tight_layout()
    
    save_path = os.path.join(output_dir, 'pipeline_demo.png')
    plt.savefig(save_path, dpi=200, bbox_inches='tight')
    plt.show()
    
    logging.info(f"✓ Demo visualization saved: {save_path}")

# Create demo visualization
create_demo_visualization(analyzer, test_crops, OUTPUT_DIRS["demo"], n_samples=6)

# %% 
# Additional: Detailed single image demo
def create_detailed_single_demo(analyzer, image_path, output_dir):
    """Create a detailed visualization for a single image"""
    logging.info(f"\nCreating detailed demo for: {image_path}")
    ...
    ax2.imshow(img_boxes)
    ax2.set_title(f'Detected Lesions: {result["num_lesions"]}', fontsize=12, fontweight='bold')
    ax2.axis('off')
    
    # 3-5. Individual crops
    pil_img = Image.fromarray(img_rgb)
    for i, (x1, y1, x2, y2, conf) in enumerate(result['boxes'][:3]):
        ax = plt.subplot(2, 4, 3 + i)
        crop = pil_img.crop((x1, y1, x2, y2))
        ax.imshow(crop)
        pred_id = result['crop_preds'][i]
        pred_name = LABELS[pred_id]['name']
        pred_conf = result['crop_probs'][i][pred_id]
        ax.set_title(f'Crop {i+1}: {pred_name}\n{pred_conf:.3f}', fontsize=10)
        ax.axis('off')
    
    # 6. Bag probabilities
    ax6 = plt.subplot(2, 4, 6)
    class_names = [LABELS[i]['name'] for i in sorted(LABELS.keys())]
    colors = plt.cm.RdYlGn(result['bag_probs'])
    ax6.barh(class_names, result['bag_probs'], color=colors)
    ax6.set_xlim(0, 1)
    ax6.set_xlabel('Probability')
    ax6.set_title('Bag-Level Probabilities', fontsize=12, fontweight='bold')
    ax6.grid(True, alpha=0.3, axis='x')
    
    # 7. Crop predictions distribution
    ax7 = plt.subplot(2, 4, 7)
    crop_names = [LABELS[p]['name'] for p in result['crop_preds']]
    crop_counts = pd.Series(crop_names).value_counts()
    ax7.bar(range(len(crop_counts)), crop_counts.values, color='steelblue')
    ax7.set_xticks(range(len(crop_counts)))
    ax7.set_xticklabels(crop_counts.index, rotation=45, ha='right')
    ax7.set_ylabel('Count')
    ax7.set_title('Crop Predictions Distribution', fontsize=12, fontweight='bold')
    ax7.grid(True, alpha=0.3, axis='y')
    ...
    bag_pred_name = LABELS[result['bag_pred']]['name']
    bag_conf = result['bag_probs'][result['bag_pred']]
    
    summary = f"""
    ANALYSIS SUMMARY
    ════════════════════
    
    Detected Lesions: {result['num_lesions']}
    ...
    Crop Predictions:
      {', '.join([LABELS[p]['name'] for p in result['crop_preds']])}
    
    Aggregation:
      Majority Vote
    """
    
    ax8.text(0.1, 0.5, summary, fontsize=10, family='monospace',
            verticalalignment='center',
            bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.5))
    
    plt.suptitle(f'Detailed Analysis: {Path(image_path).name}',
                fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    save_path = os.path.join(output_dir, f'detailed_demo_{Path(image_path).stem}.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.show()
    
    logging.info(f"✓ Detailed demo saved: {save_path}")

# Create detailed demo for one sample
if len(test_crops) > 0:
    sample_parent = test_crops.iloc[0]['parent_image']
    create_detailed_single_demo(analyzer, sample_parent, OUTPUT_DIRS["demo"])

logging.info("\n✓ Phase 12 & 13 completed successfully!")